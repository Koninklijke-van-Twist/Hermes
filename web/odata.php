 ...
 * (nightly en on-demand via odata_get_all, inclusief company-discovery) naar Mímir i.p.v. BC.
 *
 * Met $mimirApi gezet zijn $auth_list / $environment / $baseUrl / $auth ongebruikt voor BC;
 * Mímir beheert environments — Hermes heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
 ...
 * Alleen als Mímir aan staat; het BC-pad blijft de bestaande variabelen eisen.
 */
...
function odata_company_url(string $environment, string $company, string $entity, array $params = []): string
{
    ...
    // Lege of ontbrekende baseUrl blijft een geldig pad voor de Mímir-vertaling.
    $base = (string) ($baseUrl ?? '') . $environment . "/ODataV4/Company('" . $encCompany . "')/";
    $query = '';
    if (!empty($params)) {
